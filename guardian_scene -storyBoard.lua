...
-- Collisions
local function onCandyCollision( self, event)
    local collided_bug = event.other ;
    print(collided_bug.i)
    print(event.phase)
    print("xxxxxxxxxxxxxxxxxx")

end
local function ArrangeGroups()
    --candy:toBack() ;
    bugs:toFront() ;
...
local function GetRandomPosition()
    local Xs={};
    Xs[1] = math.random(_G._W* -0.15 , _G._W * - 0.1);
    Xs[2] = math.random(_G._W*1.1 , _G._W *1.15);

    local Ys={};
    Ys[1] = math.random(_G._H* -0.1 , _G._H *0.0);
    Ys[2] = math.random(_G._H*1 , _G._H *1.1);
    local point={};
     point.x =Xs[ math.random(1,2)]   ;
     point.y =Ys[ math.random(1,2)]  ;
    return point ;
end
local function buildLevel(...)
    ...
    for i = 1, num_Bug do
        ...
        bug.anchorX = 0.5;
        bug.anchorY = 0.5;
        physics.addBody(bug, "static")

        local point =  GetRandomPosition();
        bug.x = point.x;
        bug.y = point.y;
       -- bug.x = _G.halfW * bugIndex;
        -- bug.y = _G.halfH ;
        -- bug.x = math.random(0, _G._W * 4)  ;
        bug.y =  _G.halfH * bugIndex;
        bug.rotation = 180 ;
        bug.xScale = 0.4 ;
        bug.yScale = 0.4 ;
        bug.id = bugIndex ;
...
local function bugMovement1(bugObject, delayTime)
    local target = bugObject
    local _delay = delayTime
    -- local _X = math.random(_G.halfW*1.9, _G.halfW*2.1)
    --  _Y = math.random(_G.halfH*1.9, _G.halfH*2.1)
     local _X = candy.x  ;
     local _Y = candy.y  ;
    ...
    local angleBetween = mCeil(mAtan2( (_Y - y), (_X - x) ) * 180 / mPi) +90
...
local function cruch_Me1(bugObject)
    ...
    dead_bug.yScale = target.yScale  ;
    dead_bug.id = bugIndex;
...
local function cruch_Me(bugObject , touchType)
    ...
    if touchType == "touch" then
            ...
            dead_bug_1.xScale =  target.xScale ;
            dead_bug_1.yScale = target.yScale  ;
            dead_bug_1.id = bugIndex;
            dead_bug_1:toBack();
            ...
            dead_bug_2.xScale = target.xScale  ;
            dead_bug_2.yScale = target.yScale  ;
            dead_bug_2.id = bugIndex;
            dead_bug_2:toBack();
...
function scene:enterScene(event)
   ...
   -- elseif (phase == "did") then
        ...
        -- Set up collisions
        physics.addBody(candy, "static")
      --  candy.collision = onCandyCollision
      --  candy:addEventListener( "collision", candy )
        candy:addEventListener( "collision", onCandyCollision )
...
function scene:destroyScene(event)
    ...
    storyboard.removeScene(_G.candyScene, true)
end
